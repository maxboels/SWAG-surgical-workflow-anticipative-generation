...
class Medical_Dataset(Dataset):
        def __init__(self, dataframe, train_mode='train', dataset_name="cholec80", video_indices=None,
            ...
            if self.predict_next_segment:
                ...
                self.next_labels_frequencies = torch.zeros((self.num_classes+1,)).float().to(self.device)
                for video_labels in self.next_segments_class:
                    # first element is first next class.
                    self.next_labels_frequencies += torch.bincount(video_labels[:, 0].to(self.device), minlength=self.num_classes+1).float()
                self.next_classes_weights = 1 / (self.next_labels_frequencies + 1)

...
def get_sweep_param_from_combinations(clis):
    ...
    # print('[LAUNCH] Res params comb: ', res)
    return res
...
def get_sweep_param_from_runs(conf_path):
    ...
    # print('[LAUNCH] Res params runs: ', res)
    return res
...
def construct_cmd(args):
    ...
    run_id, cli_stuff = read_file_into_cli(CODE_DIR+'/'+args.cfg,
    ...
    cli = (
        ...
        ' {} {}/train_net.py hydra.run.dir={} ').format(
            'kernprof -l ' if args.profile else 'python ',CODE_DIR, agent_folder)
    cli += cli_stuff
    if args.test:
        cli += ' test_only=True '
    if args.local:

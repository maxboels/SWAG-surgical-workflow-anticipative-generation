            ...
            for i in range(max_num_steps):
                ...
                if self.fixed_ctx_length:
                    if dec_in.size(1) == self.num_ctx_tokens:
                        dec_in = dec_in[:, 1:]
                        print(f"[R2A2] trim context length by 1 on the left side")
                # else:
                #     dec_in = dec_in[:, 1:]
                ...
                if torch.argmax(next_frame_cls, dim=-1)[0, 0] == self.eos_class:
                    print(f"[R2A2] EOS token predicted at step {i}, stopping inference")
                    break

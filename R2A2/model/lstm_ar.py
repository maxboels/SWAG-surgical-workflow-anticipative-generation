    ...
    def forward(self, obs_video, train_mode=True,
        ...
        if train_mode:
            ...
            for _ in range(max_num_steps):
                # LSTM decoder
                next_frames, (h0, c0) = self.frame_decoder(dec_in, (h0, c0))
                print(f"[LSTM] next_frames: {next_frames.shape}")
                next_frame_embed = next_frames[:, -1:, :]
                print(f"[LSTM] next_frame_embed: {next_frame_embed.shape}")
            ...
            print(f"[LSTM] future_frames: {outputs['future_frames'].shape}")
